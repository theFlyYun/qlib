...
def apply_bucket_ranking(
    ...
    selected = select_bucketed_top(ranked, ranking_config, int(config["report"]["top_n"]))
...
def select_bucketed_top(predictions: pd.DataFrame, ranking_config: dict[str, Any], top_n: int) -> pd.DataFrame:
    ...
    if shortfall > 0:
        for bucket in ranking_config.get("refill_order", BUCKET_ORDER):
            if shortfall <= 0:
                break
            candidates = bucket_candidates(predictions, bucket, selected_symbols)
            chosen = candidates.head(shortfall)
            if chosen.empty:
                continue
